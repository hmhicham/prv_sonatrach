...
function Home({ username, isManager }) {
  ...
  const handleSavePolygon = async () => {
    ...
    try {
      ...
      if (positions && Array.isArray(positions)) {
        ...
        console.warn('No valid positions in response, using currentShape:', currentShape);
        setDrawnShapes((prev) => [...prev, { positions: currentShape, created_by }]);
      }
  ...
  // Function to handle polygon click and redirect to PolygonDetails
  const handlePolygonClick = (concessionName) => {
    navigate(`/polygon-details/${encodeURIComponent(concessionName)}`);
  };
                ...
                concessions
                  ...
                  .map((concession) => (
                    <Polygon
                      ...
                      pathOptions={{ color: '#FF9800' }}
                      eventHandlers={{
                        click: () => handlePolygonClick(concession.name), // Redirect on click
                      }}
                    >
                      <Popup>
                        <div>{concession.name}</div>
